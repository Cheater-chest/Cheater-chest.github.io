...
浏览器打开 [http://127.0.0.1:4000](http://127.0.0.1:4000)，修改文件后会自动增量刷新。
...
框架已内置 **KaTeX**（CDN 引入，GitHub Pages 无需额外配置）：

|需求|写法|说明|
|-|-|-|
|行内公式|`\\(E = mc^2\\)`|避免与 Markdown 语法冲突|
|块级公式|`$$ \\sum\_{i=1}^{n} i = \\frac{n(n+1)}{2} $$`|单独成段|

洛谷博客部分特性在 Jekyll 中的对应写法：

|洛谷语法|Jekyll 写法|效果|
|-|-|-|
|`:::tip 内容`|`> 内容` 下一行写 `{: .tip}`|绿色提示块|
|`:::warning`|`> 内容` + `{: .warning}`|黄色警告块|
|`:::danger`|`> 内容` + `{: .danger}`|红色注意块|
|`:::success`|`> 内容` + `{: .success}`|蓝色成功块|
|`<details><summary>`|原样支持|折叠块|
|```cpp 代码块|原样支持|代码高亮|

> 洛谷渲染器是私有实现，`:::xxx` 语法 Jekyll 不解析，需按上表改写；公式与折叠块可直接沿用。
